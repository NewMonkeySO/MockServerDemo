...
import { postReservationsCandidatesRequestSchema, postReservationsRequestSchema, putReservationsIdPaymentRequestSchema, putReservationsIdRequestSchema } from '../../schemas/reservations/index.js';
...
reservations.post('/reservations/candidates', zValidator('json', postReservationsCandidatesRequestSchema, (result, c) => {
    if (!result.success) {
...
/** POST /reservations */
reservations.post('/reservations', zValidator('json', postReservationsRequestSchema, (result, c) => {
    if (!result.success) {
        return c.json(BAD_REQUEST_RESPONSE, 400);
    }
}), async (c) => {
...
/** PUT /reservations/:id */
reservations.put('/reservations/:id', zValidator('json', putReservationsIdRequestSchema, (result, c) => {
    if (!result.success) {
        return c.json(BAD_REQUEST_RESPONSE, 400);
    }
}), async (c) => {
...
reservations.get('/reservations/:id/payment', async (c) => {
    const id = c.req.param('id');
    console.log("Path parameters:", id);
    //401
    //403
...
reservations.put('/reservations/:id/payment', zValidator('json', putReservationsIdPaymentRequestSchema, (result, c) => {
    if (!result.success) {
        return c.json(BAD_REQUEST_RESPONSE, 400);
    }
}), async (c) => {
