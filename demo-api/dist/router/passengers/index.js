...
import { getPassengerByIdReservationsQueryParametersSchema, getPassengerByIdServicesQueryParametersSchema, postPassengerAgreementsRequestSchema, postPassengerRequestSchema, putPassengerByIdRequestSchema } from '../../schemas/passengers/index.js';
import { BAD_REQUEST_RESPONSE, NOT_FOUND_RESPONSE } from '../../constants/errorCode/index.js';
const passengers = new Hono();
/** POST /passengers */
passengers.post('/passengers', zValidator('json', postPassengerRequestSchema, (result, c) => {
    if (!result.success) {
...
/** PUT /passengers/{id} */
passengers.put('/passengers/:id', zValidator('json', putPassengerByIdRequestSchema, (result, c) => {
    if (!result.success) {
        return c.json(BAD_REQUEST_RESPONSE, 400);
    }
}), async (c) => {
...
passengers.delete('/passengers/:id', async (c) => {
    const id = c.req.param('id');
    console.log("Received passenger id:", id);
    //401
    //403
...
passengers.get('/passengers/:id/services', zValidator('query', getPassengerByIdServicesQueryParametersSchema, (result, c) => {
    if (!result.success) {
        return c.json(BAD_REQUEST_RESPONSE, 400);
    }
}), async (c) => {
...
passengers.post('/passengers/:id/agreements', zValidator('json', postPassengerAgreementsRequestSchema, (result, c) => {
    if (!result.success) {
        return c.json(BAD_REQUEST_RESPONSE, 400);
    }
}), async (c) => {
...
passengers.get('/passengers/:id/agreements', async (c) => {
    const id = c.req.param('id');
...
/** @TODO パラメーターに合わせてレスポンスを修正する */
passengers.get('/passengers/:id/reservations', zValidator('query', getPassengerByIdReservationsQueryParametersSchema, (result, c) => {
    if (!result.success) {
        return c.json(BAD_REQUEST_RESPONSE, 400);
    }
}), async (c) => {
